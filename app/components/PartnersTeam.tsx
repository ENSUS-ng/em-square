    ...
    >
      <div className="flex flex-col gap-10">
        ...
        <div className="overflow-hidden rounded-4xl border border-white/10 bg-white/5 px-4 py-5">
          <div className="flex min-h-[96px] w-[220%] items-center gap-6 animate-marquee whitespace-nowrap">
            {[...partnerLogos, ...partnerLogos].map((partner, index) => (
              <div
                key={`${partner.name}-${index}`}
                className="inline-flex min-w-[220px] items-center justify-between rounded-3xl border border-white/10 bg-slate-950/30 px-5 py-4 text-sm text-slate-200"
              >
